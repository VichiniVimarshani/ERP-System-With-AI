...
const TeamAddModal: FC<ICustomerEditModalProps> = ({ id, isOpen, setIsOpen }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                axios
                    .get(`http://localhost:8090/team/${id}`)
                    .then((res: any) => {
                        setTeam(res.data)
                        console.log(res.data.employees)
                        setSelectedEmployeeData(res.data.employees)
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            ...
            if (!team.email) {
                ...
                !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(team.email)
            ) {
        ...
        onSubmit: async (values) => {
            ...
            team.employees = selectedEmployeeData
            console.log(team)

             await putservice(`team/${id}`, team)
            .then((res:any)=>{
    ...
    const handleLeaderBlur = (event: any) => {
        ...
        if (selectedLeader) {
            ...
            team.leader = selectedLeader.name
            team.email = selectedLeader.email
        ...
        <Modal isOpen={isOpen} setIsOpen={setIsOpen} size='xl' titleId={id}>
            ...
            <ModalBody className='px-4'>
                ...
                <div className='row g-4'>
                    ...
                    <FormGroup id='TeamName' label='Team Name' onChange={formik.handleChange} className='col-md-6'>
                        <Input
                            onChange={(e: any) => { team.TeamName = e.target.value }}
                            value={team?.TeamName}
                    ...
                    <FormGroup id='leader' label='Leader' onChange={(e: any) => { team.leader = e.target.value }} className='col-md-6'>
                        <Input
                    ...
                    <FormGroup id='email' label='Email' onChange={(e: any) => { team.email = e.target.value }} className='col-md-6'>
                        <Input
                            required
                            type='email'
                            onChange={formik.handleChange}
                                                        ...
                                                        <th>
                                                            <input
                                                                ...
                                                                id={`checkbox_${employee.id}`}
                                                                  checked={selectedEmployeeData?.some((e) => e._id === employee._id)}
